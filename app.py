...
def main():
    st.markdown("<h1 style='text-align: center; color: red;'>Fashion Retailer : Text to Image Search </h1>", unsafe_allow_html=True)

    multimodal_search = MultimodalSearch()

    query = st.text_input("Enter your query like Womens Fashion/Mens Clothing/Blue Denim etc:")
    if st.button("Search"):
        if len(query) > 0:
